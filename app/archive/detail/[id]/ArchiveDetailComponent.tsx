...
const FileViewer = ({ file_path, content }: { file_path: string | null, content: string | null }) => {
  if (content) {
    return (
      <div className="prose max-w-none">
        <pre className="whitespace-pre-wrap bg-gray-50 p-4 rounded-lg">{content}</pre>
      </div>
    );
  }
  ...
  if (fileExtension === 'pdf') {
    return (
      <div className="w-full h-[800px] border rounded-lg">
        <iframe src={fileUrl} width="100%" height="100%" title={file_path}></iframe>
      </div>
    );
  }
  ...
  if (['png', 'jpg', 'jpeg', 'gif'].includes(fileExtension || '')) {
    return (
      <div className="w-full">
        <img src={fileUrl} alt={file_path} className="max-w-full h-auto rounded-lg" />
      </div>
    );
  }

  return <p className="text-gray-500">지원하지 않는 파일 형식입니다. <a href={fileUrl} download className="text-blue-600 hover:underline">파일 다운로드</a></p>;
};


export default function ArchiveDetailComponent({ id }: ArchiveDetailComponentProps) {
  ...
  useEffect(() => {
    if (id) {
      async function fetchDocument() {
        try {
          setLoading(true);
          await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/documents/${id}/view`, { method: 'PUT' });
  ...
  const getCategoryColor = (category: string) => {
    switch (category.toLowerCase()) {
      ...
      case 'poster': return 'bg-green-100 text-green-600';
      case 'video': return 'bg-purple-100 text-purple-600';
      case 'db': return 'bg-yellow-100 text-yellow-600';
  ...
  const handleDownload = async () => {
    ...
    try {
      await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/documents/${document.id}/download`, { method: 'PUT' });
    } catch (err) {
      console.error('다운로드 횟수 증가에 실패했습니다.', err);
    }
    const fileUrl = `http://127.0.0.1:5001/uploads/${document.file_path}`;
    const link = document.createElement('a');
    link.href = fileUrl;
    link.download = document.file_path;
  ...
  const handleReport = () => {
    if (!reportReason || !reportDescription.trim() || !document) {
    ...
    localStorage.setItem(`report_${id}_${Date.now()}`, JSON.stringify(reportData));
  ...
  const isAuthor = user && document && user.name === document.author;
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/documents/${document.id}`, {
        method: 'DELETE',
        headers: {
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <nav className="mb-8">
          <div className="flex items-center space-x-2 text-sm text-gray-500">
            <Link href="/" className="hover:text-blue-600 cursor-pointer">홈</Link>
            <i className="ri-arrow-right-s-line"></i>
            <Link href="/archive" className="hover:text-blue-600 cursor-pointer">자료실</Link>
            <i className="ri-arrow-right-s-line"></i>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
          ...
          <div className="p-8 border-b border-gray-200">
            <div className="flex items-start justify-between mb-4">
              ...
              {isAuthor && (
                <div className="flex items-center space-x-2">
                  <button 
                    onClick={() => router.push(`/upload?edit=${document.id}`)}
            ...
            <div className="flex items-center space-x-6 text-gray-600 mb-6">
              <div className="flex items-center">
                ...
                <span>{document.author} ({document.grade})</span>
              </div>
              <div className="flex items-center">
          ...
          <div className="p-8">
            ...
            {/* File Viewer */}
            <div className="mb-8">
              <h3 className="text-xl font-semibold text-gray-900 mb-4">자료 내용</h3>
              <FileViewer file_path={document.file_path} content={document.content} />
            </div>
          ...
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            ...
            <div className="space-y-3">
              ...
              >
                ...
                {copySuccess && <span className="ml-auto text-green-600 text-sm">복사됨!</span>}
              </button>
            </div>
          </div>
        </div>
