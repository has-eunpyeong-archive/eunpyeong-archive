...
function ArchiveContent() {
  ...
  useEffect(() => {
    async function fetchArchives() {
      ...
      try {
        ...
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/documents?${queryParams}`);
        if (!res.ok) {
  ...
  const handlePageChange = (pageNumber: number) => {
    if (pageNumber > 0 && pageNumber <= totalPages) {
      ...
      router.push(`/archive?page=${pageNumber}&category=${activeCategory === '전체' ? '' : activeCategory}&search=${searchTerm}&sort_by=${sortBy}`);
    }
  };
  ...
  const handleCategoryChange = (categoryName: string) => {
    ...
    router.push(`/archive?page=1&category=${categoryName === '전체' ? '' : categoryName}&search=${searchTerm}&sort_by=${sortBy}`);
  };
  ...
  const handleSearchSubmit = () => {
    ...
    router.push(`/archive?page=1&category=${activeCategory === '전체' ? '' : activeCategory}&search=${localSearchTerm}&sort_by=${sortBy}`);
  };
  ...
  const handleSortByChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    router.push(`/archive?page=1&category=${activeCategory === '전체' ? '' : activeCategory}&search=${searchTerm}&sort_by=${e.target.value}`);
  };
  ...
  const getCategoryColor = (category: string) => {
    switch (category.toLowerCase()) {
      case '논문': return 'bg-blue-100 text-blue-600';
      case '포스터': return 'bg-green-100 text-green-600';
      case '영상': return 'bg-purple-100 text-purple-600';
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              ...
              <div className="space-y-2">
                {categories.map((category) => (
                  ...
                  >
                    <i className={`${category.icon} mr-3 text-lg w-5 h-5 flex items-center justify-center`}></i>
                    {category.name}
                  </button>
                ))}
              </div>
                  ...
                  <div className="p-6">
                    <div className="flex items-start justify-between mb-4">
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${getCategoryColor(item.category)}`}>
                        <i className={`${getCategoryIcon(item.category)} mr-1 text-sm w-4 h-4 flex items-center justify-center`}></i>
