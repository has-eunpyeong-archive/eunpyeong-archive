  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editId) {
        ...
        response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/documents/${editId}`, {
          method: 'PUT',
        ...
        response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/documents`, {
          method: 'POST',
          headers: { 'Authorization': `Bearer ${token}` },
          body: data,
        });
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || (editId ? '자료 수정에 실패했습니다.' : '자료 업로드에 실패했습니다.'));
      }
        ...
        <div className="h-60 bg-gradient-to-r from-blue-500 to-blue-600 rounded-b-[60px] mx-4 sm:mx-6 lg:mx-8 mt-4 overflow-hidden">
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="text-white text-center">
              <h1 className="text-4xl lg:text-5xl font-bold mb-4">{editId ? '자료 수정' : '자료 업로드'}</h1>
              <p className="text-xl opacity-90">{editId ? '문서 정보를 수정합니다.' : '새로운 자료를 공유해보세요'}</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              ...
              <input
                ...
                placeholder="자료 제목을 입력하세요" required
              />
            </div>
            ...
            <div>
              <label htmlFor="description" className="block text-sm font-medium text-gray-700 mb-2">설명 *</label>
              <textarea
                ...
                placeholder="자료에 대한 설명을 입력하세요 (최대 500자)" required
              />
              <div className="text-right text-sm text-gray-500 mt-1">{formData.description.length}/500</div>
            </div>
            ...
            <div>
              ...
              {editId && (
                <div className="text-sm text-blue-600 bg-blue-50 p-3 rounded-lg mb-4">
                  ...
                  기존 파일을 유지하려면 아무것도 업로드하지 마세요. 파일을 교체하려면 새 파일을 업로드하세요.
                </div>
              )}

              {/* The actual file upload dropzone, now visible in both modes */}
              <div
                className={`relative border-2 border-dashed rounded-lg p-8 text-center transition-colors ${dragActive ? 'border-blue-400 bg-blue-50' : 'border-gray-300 hover:border-blue-400'}`}
              ...
              >
                ...
                {formData.file ? (
                  <div className="space-y-2">
                    <div className="w-12 h-12 mx-auto flex items-center justify-center bg-blue-100 rounded-full"><i className="ri-file-text-line text-blue-600 text-xl"></i></div>
                    <p className="text-sm font-medium text-gray-900">{formData.file.name}</p>
                    <p className="text-xs text-gray-500">{(formData.file.size / 1024 / 1024).toFixed(2)} MB</p>
                  </div>
                ) : (
                  <div className="space-y-2">
                    <div className="w-12 h-12 mx-auto flex items-center justify-center bg-gray-100 rounded-full"><i className="ri-upload-cloud-2-line text-gray-400 text-xl"></i></div>
                    <p className="text-sm text-gray-600">파일을 드래그하거나 클릭하여 업로드하세요</p>
                    <p className="text-xs text-gray-500">TXT, PDF, DOC, PPT, MP4 등 파일 지원</p>
                  </div>
                )}
              </div>
            </div>
            ...
            {success && <div className="text-green-500 text-sm text-center">{editId ? '자료가 성공적으로 수정되었습니다.' : '자료가 성공적으로 업로드되었습니다.'} 잠시 후 상세 페이지로 이동합니다.</div>}
            ...
            <div className="flex justify-end space-x-4 pt-6">
              ...
              >{submitting ? (editId ? '수정 중...' : '업로드 중...') : (editId ? '수정하기' : '업로드')}</button>
            </div>
          </form>
        </div>
      </div>
